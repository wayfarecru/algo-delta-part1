...
IT my_partition_hoare(IT first, IT last, PRED pred) { // Hoare partition scheme
	IT it = first;
	IT jt = last - 1;
	while (1) {
...
void my_qsort(typename std::vector<T>::iterator first, typename std::vector<T>::iterator last,
	...
	auto mid1 = my_partition_hoare(first, last, [piv_val, compare](const T& x) {
		return compare(x, piv_val);
	});
	auto mid2 = my_partition_hoare(mid1, last, [piv_val, compare](const T& x) {
