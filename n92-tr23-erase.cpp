...
template <typename T>
void tr23_fixup_after_erase(SBTREE<T>& tr, typename SBTREE<T>::iterator z) {
}

template <typename T>
void tr23_erase(SBTREE<T>& tr, typename SBTREE<T>::iterator z, int ind) {
	assert(z != tr.end());
	assert(ind == 0 || ind == 1);
	// internal node : find successor, then remove the successor
	typename SBTREE<T>::iterator r = tr.child(z, ind + 1);
	if (! ISNIL(r)) { // it is inernal node
		typename SBTREE<T>::iterator s = tr23_find_min(tr, r);
		(*z)[ind] = (*s)[0];
		z = s;
		ind = 0;
	}
	// now, we want to erase a terminal node
	if (tr.number_of_children(z) == 3) { // for 3-node, just merge it
		assert((*z).size() == 2);
		if (ind == 0) {
			(*z).pop_front();
			typename SBTREE<T>::iterator w = tr.child(z, 0);
			tr.erase(w);
		} else { // ind == 1
			(*z).pop_back();
			typename SBTREE<T>::iterator w = tr.child(z, 1);
			tr.erase(w);
		}
		assert((*z).size() == 1);
		assert(tr.number_of_children(z) == 2);
		assert(ISNIL(tr.chlid(z, 0)) == true);
		assert(ISNIL(tr.chlid(z, 1)) == true);
		return;
	}
	// for 2-node, it is a hole !
	assert((*z).size() == 1);
	(*z).clear(); // remove data
	typename SBTREE<T>::iterator w = tr.child(z, 1);
	tr.erase(w);
	assert(tr.number_of_children(z) == 1);
	tr23_fixup_after_erase(tr, z);
}

template <typename T>
void tr23_erase(SBTREE<T>& tr, const T& key) {
	std::pair<typename tree<T>::iterator, int> p = tr23_find(key);
	if (p.first != tr.end()) {
		tr23_erase(tr, p.first, p.second);
	}
}
