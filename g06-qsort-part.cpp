...
IT my_partition(IT first, IT last, PRED pred) { // Lomuto partition scheme
	while ((first != last) && pred(*first)) { // equivalently: first = std::find_if_not(first, last, pred);
		++first;
	}
	if (first == last) {
		return first;
...
void my_qsort(typename std::vector<T>::iterator first, typename std::vector<T>::iterator last,
	...
	auto mid1 = my_partition(first, last, [piv_val, compare](const T& x) {
		return compare(x, piv_val);
	});
	auto mid2 = my_partition(mid1, last, [piv_val, compare](const T& x) {
