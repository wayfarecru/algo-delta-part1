...
template<typename IT, typename Compare = std::less<std::iter_value_t<IT>>>
void my_qsort(IT first, IT last, const Compare& compare = Compare()) {
	using T = std::iter_value_t<IT>; // if needed
	...
	auto mid1 = std::partition(first, last, [piv_val,compare](const auto& x) {
		return compare(x, piv_val);
	});
	auto mid2 = std::partition(mid1, last, [piv_val,compare](const auto& x) {
		return ! compare(piv_val, x);
	});
	my_qsort(first, mid1, compare);
	my_qsort(mid2, last, compare);
}
...
void my_test(std::vector<T>& A, const Compare& compare = Compare()) {
	...
	std::copy(A.begin(), A.end(), B.begin());
	{
		...
		my_qsort(B.begin(), B.end(), compare);
