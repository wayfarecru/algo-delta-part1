...
void heapify(std::vector<T>& A, int n, int i) {
	int largest = i;
	if (int l = Left(i); (l < n) && (A[largest] < A[l])) {
		largest = l;
	}
	if (int r = Right(i); (r < n) && (A[largest] < A[r])) {
		largest = r;
	}
	if (largest != i) {
...
int main(int argc, char* argv[]) {
	...
	print(A);
	cout << "is_heap(A) = " << std::boolalpha << std::is_heap(A.begin(), A.end()) << endl;
	{
	...
	print(C);
	cout << "is_heap(C) = " << std::boolalpha << std::is_heap(C.begin(), C.end()) << endl;
	// diff
