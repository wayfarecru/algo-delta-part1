...
void my_cnt10_sort(const std::vector<int>& A, std::vector<int>& B, std::vector<int>& C, const int radix) {
...
void my_radix_sort(std::vector<int>& A, const int K) {
	std::vector<int> B(A.size());
	std::vector<int> C(10);
	int dir = 0;
	for (int i = 1; i <= K; i *= 10) {
		if (dir == 0) {
			my_cnt10_sort(A, B, C, i);
		} else {
			my_cnt10_sort(B, A, C, i);
		}
		dir = (1 - dir);
	}
	if (dir == 1) { // data is in B
