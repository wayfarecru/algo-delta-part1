...
int main(int argc, char* argv[]) {
	...
	auto mid = partition(B.begin(), B.end(), [p = B[0]](int x) {
		return (x < p);
	});
	cout << "B[" << B.size() << "] = ";
	print<int>(B.begin(), mid);
	cout << "| ";
	print<int>(mid, B.end());
	cout << endl;
	...
	int thres = C[0];
	mid = stable_partition(C.begin(), C.end(), [p = C[0]](int x) {
		return (x < p);
	});
	cout << "C[" << C.size() << "] = ";
	print<int>(C.begin(), mid);
	cout << "| ";
	print<int>(mid, C.end());
	cout << endl;
	// check it
	cout << "\tis_partitioned(C) = " << std::boolalpha << std::is_partitioned(C.begin(), C.end(), [p = thres](int x) {
		return (x < p);
	}) << endl;
	auto it = partition_point(C.begin(), C.end(), [p = thres](int x) {
		return (x < p);
	});
	cout << "\tpartitoin_point(C) index = " << std::distance(C.begin(), it) << endl;
	// done
	return 0;
}
