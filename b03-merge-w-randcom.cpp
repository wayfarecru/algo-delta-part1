#include <iostream>
#include <random> // for random_engine and uniform_int_distribution
...
template<typename T>
void merge(std::vector<T>& A, std::vector<T>& B, int p, int q, int r) {
	// copy A[p..r) to B[p..r)
	for (int i = p; i < r; ++i) {
		B[i] = A[i];
	}
...
template<typename T>
void merge_sort(std::vector<T>& A, std::vector<T>& B, int p, int r) {
	if ((r - p) <= 1) return; // if 0 or 1 element, just return
	int q = (p + r) / 2; // mid point
	merge_sort(A, B, p, q); // sort A[p..q)
	merge_sort(A, B, q, r); // sort A[q..r)
	merge(A, B, p, q, r);
}
